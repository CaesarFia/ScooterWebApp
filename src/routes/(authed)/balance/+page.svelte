...
<div class="h-screen bg-background">
	<div id="header" class="relative flex h-16 w-screen flex-row items-center justify-between rounded-b-xl border-2 backdrop-blur-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300">
	  ...
	  <div id="Profile Link" class="mr-0">
		<a href="/profile" class="relative flex">
		  ...
		  <div class="mr-1 rounded-r-lg border border-gray-600 bg-background p-1 text-gray-900">{data.user.firstname} {data.user.lastname}</div>
		</a>
	  </div>
	</div>
	<div id="currentBalance" class="relative flex h-screen flex-col items-center bg-inherit">
	  <div class="mt-64 text-8xl">{data.user.balance ? "$"+data.user.balance : "Employees don't have money!"}</div>
	...
	<form id="paymentFooter" class="relative flex h-1/2 flex-col items-center justify-center space-y-1 bg-white p-5" action="?/add_balance" method="POST">
	  <div class="relative flex">
		...
		<input placeholder="Amount:" 
			...
			step=.01
			required
			class="rounded-r-lg border bg-white py-3 pl-4 pr-11 text-gray-700 focus:border-emerald-300 focus:outline-none focus:ring focus:ring-emerald-200 focus:ring-opacity-40 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300" />
			</div> 
	  ...
	  <button class=" w-1/2 rounded-lg border bg-white px-11 py-3 text-gray-700 focus:border-emerald-300 focus:outline-none focus:ring focus:ring-emerald-200 focus:ring-opacity-40 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300">Pay amount</button>
	</form>
  </div>
